...
/** 兼容扁平 {name,arguments} 与 OpenAI 嵌套 {function:{name,arguments}} */
function normalizeToolCall(raw: ToolCall | Record<string, unknown>): ToolCall {
  const r = raw as Record<string, unknown>;
  const fn = (r.function as Record<string, unknown> | undefined) ?? undefined;
  const args = r.arguments ?? fn?.arguments ?? "{}";
  return {
    id: String(r.id ?? ""),
    type: String(r.type ?? "function"),
    name: String(r.name ?? fn?.name ?? "unknown"),
    arguments: typeof args === "string" ? args : JSON.stringify(args ?? {}),
  };
}

// 工具调用展示组件（简化版，用于 assistant 消息内）
const ToolCallDisplay: React.FC<{ toolCall: ToolCall }> = ({ toolCall: raw }) => {
