"""
端到端（E2E）MCP 协议测试：不 mock Server，而是真的拉起进程，用「假客户端」按 JSON-RPC 对话，验证接线是否符合 MCP 约定

模拟真实 MCP Client（Claude / Cursor）连本地 Server 时的完整握手：
initialize → notifications/initialized → tools/list / tools/call
验证一个真实 MCP Client 是否可以通过 stdio 通信方式，完整调用 MCP Server，并且 JSON-RPC 协议流程是否正确。
关注点是 协议层（wire-level）：
    进程能起来
    JSON-RPC 能来回
    Tool 注册对
...
pytest测试程序
        ...
        ↓
src.mcp_server.server (真实MCP服务器)
        |
        ↓
MCP协议层
        |
...
import json
import os
import subprocess # 在py中启动、管理和通信另一个外部进程(子进程), 使用pipe进行通信
# 使用的是 匿名管道（anonymous pipe） + stdin/stdout 重定向 + JSON-RPC over stdio 协议。

import sys
import threading # 用于创建后台线程。因为读取 stdout 会阻塞
import time
...
def _start_server() -> subprocess.Popen:
    ...
    return subprocess.Popen(
        [sys.executable, "-m", "src.mcp_server.server"], # 启动src.mcp_server.server
