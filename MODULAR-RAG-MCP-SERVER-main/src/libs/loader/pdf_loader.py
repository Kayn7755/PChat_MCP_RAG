"""
把 PDF 文件转换成可处理的 Markdown 文本，同时提取 PDF 中的图片，并生成统一的 Document 对象。
...
import hashlib # 计算 SHA256, 相当于md5标记pdf
import logging
from pathlib import Path
from typing import Any, Dict, List, Optional

try:
    from markitdown import MarkItDown # 微软的文档转换工具
    # 将pdf转为md文件
...
# 实现一个PDF加载器, 将PDF二进制文件转为RAG可用的数据结构(自定义的Document格式)
class PdfLoader(BaseLoader):
    ...
    def load(self, file_path: str | Path) -> Document:
        ...
        try: # 用MarkItDown解析PDF, 将PDF转化为MD
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, img_info in enumerate(image_list):
                    try:
                        ...
                        # Create placeholder 创建图片占位符, 文本嵌入时需要知道这里有图片
