"""
根据配置文件动态创建不同的LLM实例, 并统一管理文本LLM与视觉多模态LLM

用于创建 LLM 提供程序实例的工厂。
此模块实现了工厂模式，可根据配置实例化相应的 LLM 提供程序，从而实现基于配置的后端选择，
无需修改代码。
...
class LLMFactory:
    ...
    _PROVIDERS: dict[str, type[BaseLLM]] = {} # 文本LLM[模型名->模型类]
    ...
    _VISION_PROVIDERS: dict[str, type[BaseVisionLLM]] = {} # 视觉-文本大模型
    ...
    def register_provider(cls, name: str, provider_class: type[BaseLLM]) -> None:
        ...
        """
        if not issubclass(provider_class, BaseLLM): # 类型检查
        # provider_class是否继承BaseLLM
            raise ValueError(
