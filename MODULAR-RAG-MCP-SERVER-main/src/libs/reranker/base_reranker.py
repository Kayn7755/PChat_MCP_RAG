...
重排序组件的抽象接口层：不负责具体怎么打分，只规定「所有 Reranker 必须长什么样」，并提供一个什么都不做的默认实现。

检索 = 找候选；重排序 = 在候选里精细排名。
粗排管“别漏”，精排管“谁最该排第一”。
重排序（Rerank） 就是：粗排先捞一批“可能相关”的候选，再用更准、更贵的模型，按「查询 ↔ 这段文本」的相关性重新排一遍顺序。
混合检索（Dense + BM25 + RRF）的目标是快、召回宽，但排序不一定最准。
