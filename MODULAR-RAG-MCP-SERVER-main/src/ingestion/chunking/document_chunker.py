...
记录 chunk_index（第几块）
用 source_ref 指回父文档
把纯字符串转成 core.types.Chunk

核心增值（相对于 libs.splitter(真正实现递归切分)）：
1. Chunk ID 生成：为每个 Chunk 生成确定且唯一的 ID
2. 元数据继承：将文档元数据传播到所有 Chunk
3. chunk_index：记录 Chunk 在文档中的顺序位置
4. source_ref：建立父子可追溯性
5. 类型转换：str → Chunk 对象（core.types 契约）

设计原则：
- 适配器模式：连接文本分割工具和业务对象
- 配置驱动：使用 SplitterFactory 进行基于配置的策略选择
- 确定性：同一文档重复生成相同的 Chunk ID拆分
...
class DocumentChunker:
    ...
    def __init__(self, settings: Settings):
        ...
        self._splitter = SplitterFactory.create(settings) # 通过工厂模式创造的分割器
