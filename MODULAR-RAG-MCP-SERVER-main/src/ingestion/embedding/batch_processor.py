"""
RAG 文档入库流程（Ingestion Pipeline）中的批处理协调器（Batch Processor）。
把文档切分后的 Chunk 批量送入 Dense Encoder 和 Sparse Encoder，生成向量表示和关键词统计，并负责批次管理、异常处理、性能统计。

用for循环一次一个chunk速度慢, 批量处理速度块
...
# 保存一次批处理结果(包含了整个chunk)
@dataclass
class BatchResult:
    dense_vectors: List[List[float]]
    sparse_stats: List[Dict[str, Any]]
    batch_count: int # 总共分了多少批(1000个chunk, batch_size = 100, 那么就是10)
    total_time: float # 整个处理耗时
    successful_chunks: int # 处理成功的chunk数量
    failed_chunks: int

# 协调DenseEncoder与SparseEncoder
class BatchProcessor:
    ...
    def __init__(
        ...
        sparse_encoder: SparseEncoder,
        batch_size: int = 100, # 每批处理100个chunk
    ):
