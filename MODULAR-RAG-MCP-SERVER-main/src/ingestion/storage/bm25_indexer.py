"""
BM25 关键词检索的倒排索引: 入库时把 SparseEncoder 产出的词频统计建成可查询索引，落盘，并在检索时打分
倒序依据 = BM25 score（越大越相关）(倒序: 从大到小)
以json格式落盘, 躺在磁盘文件夹里，不是进数据库表
...
索引落成本地 JSON 文件
{
  ...
  "index": {
    "hello": {
      "idf": 0.51,          # 逆文档频率：词越稀有，权重越高
      "df": 2,              # 有多少篇文档包含这个词
      "postings": [         # 倒排列表：哪些块出现了这个词
        {"chunk_id": "doc1_0000_xxx", "tf": 2, "doc_length": 10},
        {"chunk_id": "doc2_0001_yyy", "tf": 1, "doc_length": 8}
      ]
    }
  }
}
...
class BM25Indexer:
    ...
    def __init__(
        ...
        k1: float = 1.5, # 词频饱和系数，默认 1.5。tf 再大，贡献也会封顶
        b: float = 0.75, # 长度归一化系数，默认 0.75。长文档会被压分，避免“字多就容易命中”
    ):
    ...
    # 保存BM25索引到本地JSON文件, 落盘产物是本地 JSON
...
"""
打分步骤:
打分不扫全库，而是：拿查询词去倒排表里找 posting，每个命中块算一次 BM25，多词分数相加，再按总分排序。
...
简历一旦写了混合检索，这几问几乎必来：
为什么要 BM25，不只做向量检索？
向量擅长同义/语义；BM25 擅长专有名词、型号、API 名。两路互补，再用 RRF 融合。

BM25 相对 TF-IDF 改了什么？k1、b 是干什么的？
k1 控制词频饱和，b 做文档长度归一化。

索引存在哪？和 Chroma 什么关系？
不进数据库，JSON 倒排表在 data/db/bm25/；Chroma 存稠密向量。删文档必须四路一起清（Chroma / BM25 / 图片 / 文件哈希）。

增量入库时 IDF 怎么更新？
add_documents 会先按 doc_id 删旧 posting，再和旧语料合并后 整库重算 IDF（因为 N、df 变了）。

这些够撑住「我做过 Hybrid Search」。
"""