...
class ImageStorage: # 把图片文件落盘，并建立索引
    ...
    Directory Structure:
        data/images/{collection}/{image_id}.png 真正的图片
    ...
    Args:
        db_path: Path to SQLite database file (default: data/db/image_index.db). SQLite 索引表
    ...
    def __init__(
        ...
        self.images_root = Path(images_root) # 存放图像文件的根目录。
        self._conn = None  #数据库连接, 使用sqlite3数据库
        self._ensure_database() # 确保数据库存在并创建表
