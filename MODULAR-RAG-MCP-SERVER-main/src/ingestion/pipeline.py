"""
RAG 系统的“知识库构建/文档入库模块”
负责提前把文档加工成后面能够检索的数据结构
                 PDF 文件
                    │
                    ▼
        ┌──────────────────────┐
...
# 保存一次 Pipeline 执行完成后的结果
class PipelineResult:
    """
        Pipelinet执行结果及详细统计信息。
        属性：
        success：流水线是否成功完成
        file_path：已处理文件的路径
        doc_id：文档 ID（SHA256 哈希值）
        chunk_count：生成的数据块数量
        image_count：处理的图像数量
        vector_ids：存储的向量 ID 列表
        error：流水线失败时的错误信息
        stages：阶段名称及其对应结果的字典
    """
...
class IngestionPipeline:
    ...
    - 元数据丰富（基于规则 + LLM）
    - 图像描述（Vision LLM）
    - 密集嵌入（Azure text-embedding-ada-002）
    - 稀疏编码（BM25 词项统计）
    - 向量存储（ChromaDB）
    - BM25 索引构建
    ==================================================================
    ...
    def __init__(
        ...
        # Stage 1: File Integrity 判断这个文件之前有没有处理过的函数, 还没有开始检测
        # SHA256查SQLite(给每个 PDF 文件计算一个唯一指纹（SHA256 哈希值），然后把这个哈希值存入 SQLite。如果新文件的 SHA256 已经存在，说明文件内容完全一致。)
        # SQLite 是一种轻量级关系型数据库（Relational Database），最大的特点是：不需要单独启动数据库服务器，整个数据库就是一个文件。
        # 涉及幂等, 传入多少次文件都只会保存一份
        ...
        # Stage 2: Loader 加载PDF文件, 把 PDF 转换成系统内部的 Document 对象
        self.loader = PdfLoader(
            extract_images=True, # 不仅提取文字，还提取图片
            image_storage_dir=str(resolve_path(f"data/images/{collection}"))
            # document.metadata["images"]保存图片
        )
        logger.info("  ✓ PdfLoader initialized")
        
        # Stage 3: Chunker 把整个 Document 切成多个 Chunk
        # 因为 RAG 通常不是把整本 PDF 一次性送给 Embedding 模型而是分成小块
        self.chunker = DocumentChunker(settings)
        logger.info("  ✓ DocumentChunker initialized") 
        
        # Stage 4: Transforms
        self.chunk_refiner = ChunkRefiner(settings) # 改善文本质量
        logger.info(f"  ✓ ChunkRefiner initialized (use_llm={self.chunk_refiner.use_llm})")
        
        self.metadata_enricher = MetadataEnricher(settings) # 增加标题、标签、摘要等
        logger.info(f"  ✓ MetadataEnricher initialized (use_llm={self.metadata_enricher.use_llm})")
        
        self.image_captioner = ImageCaptioner(settings) # 给 PDF 中提取出来的图片生成文字描述
        # 因为图片本身不能直接参与普通文本 Embedding, 通过VLLM生成描述再把该描述加入Chunk Metadata
        ...
        self.dense_encoder = DenseEncoder(embedding, batch_size=batch_size) # 稠密嵌入, 主要用于语义检索
        logger.info(f"  ✓ DenseEncoder initialized (provider={settings.embedding.provider})")
        
        self.sparse_encoder = SparseEncoder() # BM25 所需要的词项统计信息, 用于关键词检索
        ...
        # Stage 6: Storage 向量存储
        # 把 Chunk 和 Dense Vector 写入向量数据库
        self.vector_upserter = VectorUpserter(settings, collection_name=collection) # Dense Vector → ChromaDB
        logger.info(f"  ✓ VectorUpserter initialized (provider={settings.vector_store.provider}, collection={collection})")
        
        # Sparse → BM25 建立 Sparse / BM25 检索索引。
        # 最终形成文档-> 词项统计->BM25Index
    ...
    def run(
        ...
        try:
            ...
            # 多模态 RAG 里的图片落盘 + 索引：图片文件存在文件夹
            # 元数据存在 SQLite，方便按 image_id / 集合 / 文档查。
