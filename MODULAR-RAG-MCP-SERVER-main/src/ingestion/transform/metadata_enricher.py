"""
摄取流水线 Stage 4b：给每个 chunk 补元数据（标题、摘要、标签），方便浏览、过滤和 get_document_summary 一类工具用。
一般不改正文（和 ChunkRefiner / ImageCaptioner 不同）。
...
class MetadataEnricher(BaseTransform):
    ...
    加工流水线：
        1. 基于规则的丰富：从内容中提取基本元数据
        2.（可选）LLM丰富：生成语义丰富的元数据
        3. LLM失败时：优雅地回退到基于规则的元数据
    ...
    配置（通过settings.yaml）：
        -ingestion.metadata_enricher.use_llm: bool -启用LLM增强
        -ingestion.metadata_enricher.prompt_path: str -自定义提示文件路径
    
    设计原则：
        -优雅降级：LLM 错误不会阻止摄取
        -原子处理：每个块独立处理
        -Observable：在元数据中记录enriched_by
    """
