"""在摄取流水线的 Stage 4a：对已经切好的 chunk 做二次清洗/润色，提高进向量库的文本质量。不负责切块
，也不负责 embedding。
...
class ChunkRefiner(BaseTransform):
    """
    对已经切分好的文本 chunk 进行二次清洗和优化，让进入向量数据库的文本质量更高
    流程:
        规则清洗:
        去掉：
            多余空格
    ...
    def _refine_single_chunk(
        ...
        """
        try:
            # Step 1: Rule-based refinement # 规则清洗
            rule_refined_text = self._rule_based_refine(chunk.text)
            
            # Step 2: LLM enhancement # 大模型增强
            if self.use_llm and self.llm:
