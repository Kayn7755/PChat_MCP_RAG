"""
这个文件在摄取流水线 Stage 4c：用 Vision LLM 给 chunk 里引用的图片写文字描述，好让「图」也能被文本检索（Dense/BM25）搜到。
向量/BM25 吃的是文字。PDF 里的图若只有 [IMAGE: xxx]，检索很难命中。
描述进 chunk 文本后，语义检索和关键词检索都能用到图意。
** 通过提示词模板告诉llm的生成逻辑
...
class ImageCaptioner(BaseTransform):
    """
    使用 Vision LLM 为块中引用的图像生成标题。
    
    此转换使用 Vision LLM 识别包含图像引用的块
    生成描述性标题，并丰富块文本/元数据
    这些标题可以改善视觉内容的检索。
    
    主要特点：
    -仅处理块文本中实际引用的图像（并非元数据中的所有图像）
    -缓存字幕以避免冗余的 Vision API 调用
    -线程安全的字幕缓存，用于未来潜在的并行化
    """
