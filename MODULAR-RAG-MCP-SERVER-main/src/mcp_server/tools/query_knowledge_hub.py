"""
实现 MCP 主工具Agent 一问知识库，就走「混合检索 → 可选重排 → 带引用的回复」
MCP Tool 层的一个 RAG 查询工具实现，文件核心功能是：
将你的 RAG 检索能力封装成一个 MCP Tool，让 MCP Client（例如 Claude、Cursor、Agent 框架）可以调用 query_knowledge_hub 进行知识库搜索。

代码主要包含：
    工具元数据
    配置类 QueryKnowledgeHubConfig
    核心类 QueryKnowledgeHubTool
    单例管理函数
    MCP Handler
    Tool 注册函数
...
Usage via MCP:
    ...
    Input schema:
        - query (string, required): The search query 用户提问
        - top_k (integer, optional): Number of results to return (default: 5) 返回条数，默认 5，最大 20
        - collection (string, optional): Limit search to specific collection 知识库集合，默认 default
"""
...
# 单例模式, 获取QueryKnowledgeHubTool实例
# 避免每次都重新初始化数据库, 嵌入模型等
...
# mcp不止query_knowledge_hub这一个工具, 该代码只负责把query_knowledge_hub注册到MCP Server
# 把工具注册到 MCP Server, MCP Client 才能发现：tool_list
# protocol_handler 是 MCP Server 中负责管理协议层的对象。
# protocol_handler 就像 MCP Server 的“路由注册中心”，负责告诉 MCP 框架：有哪些 Tool、每个 Tool 的名字是什么、输入格式是什么、调用哪个函数。
