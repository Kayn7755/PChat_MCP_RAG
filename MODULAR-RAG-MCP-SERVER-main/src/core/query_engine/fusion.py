"""
把 Dense 和 Sparse 两条检索结果融成一份总排名，用的是 RRF（Reciprocal Rank Fusion）。
两路分数不可比：向量相似度可能是 0~1，BM25 可能是 0~十几。直接加会偏。
RRF 只看名次、不看原始分，两路贡献可加，也不用归一化

Dense 排名:  [A, B, C]     Sparse 排名: [B, D, A]
                │                      │
                └──────────┬───────────┘
                           ▼
                    RRFFusion.fuse()
                           │
              对每个 chunk_id 累加 1/(k+rank)
                           │
              按 RRF 分数降序，取 top_k
                           ▼
              融合后的 List[RetrievalResult]
              （score 字段已是 RRF 分，不是原分）
...
class RRFFusion:
    ...
    # 融合, 返回融合后的list[dense-rrf分数, BM25-rrf分数]
    def fuse(
        self,
        ranking_lists: List[List[RetrievalResult]], # 排名列表列表，每个列表包含 RetrievalResult
                        #    按相关性（降序）排序的对象。
                        #    通常是[密集结果，稀疏结果]
        ...
        # 按 RRF 分数（降序）排序，然后按 chunk_id 排序以获得稳定性
...
# 计算给定某个文档在某条排名列表里的名次, rank是排名, k是平滑系数
# 名次越好，贡献越大。
# 真正融合时，fuse() 会对 Dense、Sparse 各算一次再相加；这个函数只负责「单个名次 → 单个贡献值」，方便复用或单测，本身不做完整融合。
