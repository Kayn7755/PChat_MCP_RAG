"""核心层 Reranker 负责协调 libs.reranker 后端，并提供回退支持。

此模块实现了 CoreReranker 类，该类：
1. 通过 RerankerFactory 与 libs.reranker（LLM、CrossEncoder、None）集成
2. 在后端故障或超时时提供优雅的回退机制
3. 将 RetrievalResult 与 reranker 输入/输出格式相互转换
4. 支持 TraceContext 以实现可观察性

设计原则：
- 可插拔：使用 RerankerFactory 实例化已配置的后端
- 配置驱动：从 settings.yaml 读取 rerank 设置
- 优雅回退：后端故障时返回原始排序
- 可观察：集成 TraceContext 以进行调试
"""
