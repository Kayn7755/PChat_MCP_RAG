"""
稀疏检索器
用 BM25 按关键词打分，再回向量库取正文，输出和 Dense 同款的 RetrievalResult。
...
QueryProcessor 抽出的 keywords（如 ["Azure", "OpenAI", "配置"]）
                    │
                    ▼
        ...
        ┌───────────┼───────────┐
        ▼           ▼           ▼
   校验 keywords  加载 BM25   确定 top_k /
   非空、依赖齐全  JSON 索引   collection
                    │
                    ▼
         bm25_indexer.query(keywords)
                    │
                    │  倒排命中 → 算 BM25 分数 → 降序取 top_k
                    ▼
         [{chunk_id, score}, ...]
                    │
                    ▼
         vector_store.get_by_ids(chunk_ids)
                    │
                    │  取 text + metadata（BM25 本身不存正文）
                    ▼
         _merge_results：score + 正文拼成 RetrievalResult
                    │
                    ▼
         List[RetrievalResult]  → 交给 HybridSearch / RRF
"""
...
class SparseRetriever:
    ...
    此类通过以下方式执行基于关键字的检索：
    1.通过关键字查询BM25索引，得到匹配的chunk ID和分数
    2. 使用 get_by_ids() 从矢量存储中获取文本和元数据
    3.返回规范化的RetrievalResult对象
