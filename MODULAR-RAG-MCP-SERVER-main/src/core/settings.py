...
Modular RAG MCP 服务器的配置加载和验证
读取config\settings.yaml文件并校验
类似Pydantic的文件格式
"""
...
def _optional_secret(value: Any) -> Optional[str]:
    """Return a real secret string, or None for missing/placeholder values.

    Lets environment variables (e.g. OPENAI_API_KEY) take effect when
    settings.yaml still contains template placeholders.
    """
    if value is None:
        return None
    ...
    if not stripped or stripped in _PLACEHOLDER_SECRETS:
        return None
    return stripped

# 配置层要的是“轻量结构 + 最小校验”，不是完整校验框架，所以选标准库 dataclass。
...
class Settings:
    ...
    @classmethod # @classmethod 修饰 from_dict，表示这是一个类方法
    # 把「字典」转成 Settings 对象
    # cls — 类本身（你不用手动传）(相当于Settings这个类本身)
    # data — YAML 解析后的字典（你真正传入的）
    def from_dict(cls, data: Dict[str, Any]) -> "Settings":
        ...
        if "vision_llm" in data:
            ...
            vision_llm_settings = VisionLLMSettings(
                ...
                api_key=_optional_secret(vision_llm.get("api_key")),
        ...
        settings = cls(
            llm=LLMSettings(
                ...
                api_key=_optional_secret(llm.get("api_key")),
            ...
            embedding=EmbeddingSettings(
                ...
                api_key=_optional_secret(embedding.get("api_key")),
        ...
        return settings # 最后返回的settings是cls，也就是Settings的实例
